"""
3_assign_para_folder.py - Assign notes to PARA folders based on tags.

Identifies which PARA folder each note belongs to:
- Projects: notes tagged with "Project"
- Areas: notes tagged with "Area"  
- Resources: notes tagged with "Resource"
- Archive: notes without any PARA tag (orphans)

Note: The tag names are singular but folder names are plural.

Usage:
    python transforms/3_assign_para_folder.py <input_folder>
    
Example:
    python transforms/3_assign_para_folder.py 01
    # Creates output folder: 013
"""
...
def assign_para_folder(note: Note) -> str:
    ...
    Priority: Project > Area > Resource > Archive (fallback)
