"""
K_resolve_multi_parents.py - Interactive multi-parent resolver with backups.
...
2. If resolution file exists:
   ...
   - If SOME notes need work:
     * BACKUP the existing file (timestamped)
     * Rewrite file with incomplete notes at top
     * Tell user how many need work
     * EXIT

Usage:
    python transforms/K_resolve_multi_parents.py <input_folder>
    
Example:
    python transforms/K_resolve_multi_parents.py 01457A9BFGHJ
    # Creates output folder: 01457A9BFGHJK
"""
...
BACKUP_DIR = "resolve_backups"


def backup_resolution_file():
    """Create a timestamped backup of the resolution file."""
    ...
    backup_filename = f"resolve_multi_parents_{timestamp}.md"
    backup_path = backup_dir / backup_filename
    
    # Copy file
    shutil.copy2(resolution_path, backup_path)
...
def parse_resolution_file() -> dict[str, list[str]]:
    ...
    # Section headers to ignore (not note titles)
    SECTION_HEADERS = {
    ...
    for line in content.split("\n"):
        ...
        note_match = re.match(r"^#{2,3} (.+)$", line)
        if note_match:
...
def rewrite_resolution_file(notes: list[Note], complete: dict[str, str], incomplete: list[str]):
    """
...
def transform(notes: list[Note]) -> list[Note]:
    ...
    if not incomplete:
        ...
        # Clean up
        backup_resolution_file()  # Final backup before deletion
        resolution_path.unlink()
        print(f"  Cleaned up {RESOLUTION_FILE}")
        print(f"  (Final backup saved in {BACKUP_DIR}/)")
        return notes
    
    # CASE 2b: Some incomplete - reorganize file (with backup!)
    print(f"\n{len(incomplete)} notes still need a parent selected.")
    print(f"  {len(complete)} notes are complete.")
    
    # Rewrite file with incomplete at top (this will backup first)
